...
def main():
    ...
    try:
        with open(config_file, "r") as cfg_fh:
            try:
                ...
                print(
                    "Yaml file parsing unsuccessul : %s - line: %s column: %s => %s"
                    % (config_file, mark.line + 1, mark.column + 1, exc.problem)
                )
    ...
    if args.action == "exporter" and (
        not "dashboard_name" in config["general"]
    ...
    if not args.grafana_label in config["grafana"]:
        print(
            "ERROR: invalid grafana config label has been specified (-g {0}).".format(
                args.grafana_label
            )
        )
        sys.exit(1)
    ...
    if not "token" in config["grafana"]:
        print(
            "ERROR: no token has been specified in grafana config label '{0}'.".format(
                args.grafana_label
            )
        )
        sys.exit(1)
    ...
    if args.action == "import":
        ...
        if res:
            print(
                "OK: dashboard '{0}' imported into '{1}'.".format(
                    dash["title"], grafana_api.grafana_folder
                )
            )
            sys.exit(0)
        else:
            print(
                "KO: dashboard '{0}' not imported into '{1}'.".format(
                    dash["title"], grafana_api.grafana_folder
                )
            )
            sys.exit(1)
    ...
    # *******************************************************************************
        ...
        try:
            ...
            print(
                "OK: dashboard '{0}' removed.".format(
                    config["general"]["dashboard_name"]
                )
            )
        except Grafana.GrafanaDashboardNotFoundError as exp:
            print(
                "KO: dashboard '{0}' not found in '{1}".format(
                    exp.dashboard, exp.folder
                )
            )
            sys.exit(0)
            ...
            print(
                "KO: dashboard '{0}' not removed: {1}".format(
                    config["general"]["dashboard_name"], exp
                )
            )
            sys.exit(1)
        except Exception as exp:
            print(
                "error: dashboard '{0}' remove exception '{1}'".format(
                    config["general"]["dashboard_name"], traceback.format_exc()
                )
            )
            sys.exit(1)
    ...
    # *******************************************************************************
        ...
        try:
            ...
            print(
                "KO: dashboard name not found '{0}'".format(
                    config["general"]["dashboard_name"]
                )
            )
            sys.exit(1)
